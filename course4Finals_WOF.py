...
class WOFComputerPlayer(WOFPlayer):
    ...
    def __init__(self, name, difficulty):
        WOFComputerPlayer.name = name
        WOFComputerPlayer.prizeMoney = 0
        WOFComputerPlayer.prizes = []
    ...
    def getPossibleLetters(self, guessed):
        possibles = []              #getting possible guesses
        for ch in LETTERS:
            if ch not in guessed:
                if ch in VOWELS and self.prizeMoney < 250:
                    continue
                elif ch in VOWELS and self.prizeMoney >= 250:
                    possibles.append(ch)
                else:
                    possibles.append(ch)
        return possibles
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if not possibles:           #checking if no possible guesses
            return "pass"             #return pass if no possible guesses

        else: 
            if self.smartCoinFlip():        #making a good move
                prio = []
                for i in possibles:
                    rev_rank = self.SORTED_FREQUENCIES.find(i)
                    prio.append((i, rev_rank))
                    prio = sorted(prio, key = lambda tup: -tup[1])
                    unzipped = list(zip(*prio))
                    move = unzipped[0][0]
                    return move             #returning sorted guesses according to SORTED_FREQUENCIES  
            else:                           #making a bad move
